...
function validPayload(endpoint, payload) {
  ...
  if (endpoint === FEISHU_MULTI_ENDPOINTS.sendText) {
    return hasOnlyKeys(payload, new Set(['botId', 'target', 'text']))
      ...
      ? null
      : 'bot.sendText requires botId, target, and text.';
  }
...
function assertController(controller) {
  if (!controller
    ...
    || typeof controller.sendText !== 'function') {
    throw new TypeError('A Feishu controller with status/start/cancel/disconnect/sendText is required');
  }
}
...
export function createFeishuRpcHandler(controller, { encodeQr = qrCodeDataUrl } = {}) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    try {
      ...
      } else if (endpoint === FEISHU_MULTI_ENDPOINTS.sendText) {
