...
function payloadFailure(endpoint, payload) {
  ...
  if (endpoint === TOKEN_BOT_ENDPOINTS.sendText) {
...
export function createTokenBotRpcHandler(controller, { channel }) {
  for (const method of ['status', 'bindCredentials', 'reconnectBot', 'deleteBot', 'sendText']) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    try {
      ...
      } else if (endpoint === TOKEN_BOT_ENDPOINTS.sendText) {
