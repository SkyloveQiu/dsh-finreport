...
function payloadFailure(endpoint, payload) {
  ...
  if (endpoint === WECOM_ENDPOINTS.sendText) {
    return exactKeys(payload, ['botId', 'target', 'text']) && validId(payload.botId)
      && isRecord(payload.target)
      && typeof payload.text === 'string' && payload.text.length > 0 && payload.text.length <= 64 * 1024
      ? null : 'bot.sendText requires botId, target, and text.';
  }
...
export function createWecomRpcHandler(controller, { encodeQr = qrDataUrl } = {}) {
  for (const method of ['status', 'startProvisioning', 'registrationStatus', 'cancelProvisioning', 'bindCredentials', 'reconnectBot', 'deleteBot', 'sendText']) {
  ...
  return async (endpoint, payload, signal) => {
    ...
    try {
      ...
      } else if (endpoint === WECOM_ENDPOINTS.sendText) {
        value = await controller.sendText(payload.botId, payload.target, payload.text);
      } else {
