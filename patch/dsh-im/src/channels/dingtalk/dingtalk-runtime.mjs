...
export class DingtalkRuntime {
  ...
  /**
   * Push an outbound text message to a DingTalk conversation through the
   * robot's session webhook (added by the finreport integration).
   */
  async sendText(target, text) {
    if (!this.#api || !this.#client || this.#status.ready !== true) {
      throw Object.assign(new Error('DingTalk bot is not connected'), { code: 'not-connected' });
    }
    if (typeof target?.sessionWebhook !== 'string' || !target.sessionWebhook.trim()
      || typeof text !== 'string' || !text.trim()) {
      throw Object.assign(new Error('DingTalk send requires sessionWebhook and text'), { code: 'bad-target' });
    }
