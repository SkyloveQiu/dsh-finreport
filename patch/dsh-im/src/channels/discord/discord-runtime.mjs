...
export class DiscordRuntime {
  ...
  /**
   * Push an outbound text message through the live Discord API client
   * (added by the finreport integration).
   */
  async sendText(target, text) {
    const api = this.#api;
    if (!api || this.#status.ready !== true) {
      throw Object.assign(new Error('Discord bot is not connected'), { code: 'not-connected' });
    }
    if (!target || typeof target.channelId !== 'string') {
      throw Object.assign(new Error('Discord send requires a channelId'), { code: 'bad-target' });
    }
    if (typeof text !== 'string' || !text.trim()) {
      throw Object.assign(new Error('Discord send requires text'), { code: 'bad-target' });
    }
    const client = new DiscordBotClient({ api, signal: this.#abortController?.signal });
    await client.sendText({ channelId: target.channelId, replyToMessageId: target.replyToMessageId }, text);
    return { sent: true };
  }
