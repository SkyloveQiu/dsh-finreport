...
import { splitText } from './message-utils.mjs';
...
export class FeishuRuntime {
  ...
  /**
   * Push an outbound text message to an arbitrary Feishu chat through the
   * live lark client (added by the finreport integration).
   */
  async sendText(target, text) {
    if (!this.#client || this.#status.ready !== true) {
      throw Object.assign(new Error('Feishu bot is not connected'), { code: 'not-connected' });
    }
    if (!target || typeof target.receiveId !== 'string' || !target.receiveId) {
      throw Object.assign(new Error('Feishu send requires receiveId and text'), { code: 'bad-target' });
    }
