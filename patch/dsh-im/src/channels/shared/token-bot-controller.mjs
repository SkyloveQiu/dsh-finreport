...
export class TokenBotController {
  ...
  /**
   * Push an outbound text message through a connected bot runtime
   * (added by the finreport integration).
   */
  async sendText(botId, target, text) {
    const runtime = this.#runtimes.get(botId);
    if (!runtime || runtime.status?.ready !== true) {
      throw Object.assign(new Error(`${this.#descriptor.label} bot is not connected`), { code: 'not-connected' });
    }
    if (typeof runtime.sendText !== 'function') {
      throw Object.assign(new Error(`${this.#descriptor.label} runtime does not support sendText`), { code: 'unsupported' });
    }
    await runtime.sendText(target, text);
    return { sent: true, botId };
  }
