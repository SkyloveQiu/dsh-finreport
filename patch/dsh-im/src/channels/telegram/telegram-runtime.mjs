...
export class TelegramRuntime {
  ...
  /**
   * Push an outbound text message through the live Bot API client
   * (added by the finreport integration).
   */
  async sendText(target, text) {
    const api = this.#api;
    if (!api || this.#status.ready !== true) {
      throw Object.assign(new Error('Telegram bot is not connected'), { code: 'not-connected' });
    }
    if (!target || (typeof target.chatId !== 'string' && typeof target.chatId !== 'number')) {
      throw Object.assign(new Error('Telegram send requires a chatId'), { code: 'bad-target' });
    }
    if (typeof text !== 'string' || !text.trim()) {
      throw Object.assign(new Error('Telegram send requires text'), { code: 'bad-target' });
    }
    const client = new TelegramBotClient({ api, signal: this.#abortController?.signal });
