...
// Mirrors the byte-based chunking used by WecomHarnessBridge.#sendActive:
// Enterprise WeChat markdown content is capped by byte length, so split on
// UTF-8 bytes rather than code units.
function splitWecomText(text, maxBytes = 18_000) {
...
export class WecomRuntime {
  ...
  /**
   * Push an outbound text message to an arbitrary Enterprise WeChat chat
   * through the live WSClient (added by the finreport integration).
   */
  async sendText(target, text) {
    const client = this.#client;
    if (!client || this.#status.ready !== true) {
      throw Object.assign(new Error('WeCom bot is not connected'), { code: 'not-connected' });
    }
    if (typeof target?.chatId !== 'string' || !target.chatId.trim()
      || typeof text !== 'string' || !text.trim()) {
      throw Object.assign(new Error('WeCom send requires chatId and text'), { code: 'bad-target' });
    }
    for (const chunk of splitWecomText(text)) {
      await client.sendMessage(target.chatId, { msgtype: 'markdown', markdown: { content: chunk } });
    }
    return { sent: true };
  }
