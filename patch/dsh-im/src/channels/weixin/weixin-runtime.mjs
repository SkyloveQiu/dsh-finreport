import { splitWeixinText, WeixinApiError } from './weixin-api.mjs';
...
export class WeixinRuntime {
  ...
  /**
   * Push an outbound text message to an arbitrary Weixin user through the
   * live API client (added by the active-send integration).
   */
  async sendText(target, text) {
    if (!this.#api || this.#status.ready !== true) {
      throw Object.assign(new Error('Weixin bot is not connected'), { code: 'not-connected' });
    }
    if (typeof target?.toUserId !== 'string' || !target.toUserId
      || typeof text !== 'string' || !text) {
      throw Object.assign(new Error('Weixin send requires toUserId and text'), { code: 'bad-target' });
    }
