...
export class WhatsappController {
   ...
   */
  async sendText(botId, jid, text) {
    if (typeof jid !== 'string' || !jid) {
      throw Object.assign(new Error('A target jid is required'), { code: 'bad-target' });
    }
    if (typeof text !== 'string' || !text.trim()) {
      throw Object.assign(new Error('Message text is required'), { code: 'bad-target' });
    }
    ...
    await runtime.sendText({ jid, text });
