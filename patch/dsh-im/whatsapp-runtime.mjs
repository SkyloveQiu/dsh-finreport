...
export class WhatsappRuntime {
  ...
  /**
   * Push an outbound text message to an arbitrary WhatsApp jid through the
   * live Baileys socket (added by the finreport integration).
   */
  async sendText(target) {
    const client = this.#client;
    if (!client || typeof client.sendText !== 'function') {
      throw Object.assign(new Error('WhatsApp bot is not connected'), { code: 'not-connected' });
    }
    if (!target || typeof target.jid !== 'string' || !target.jid) {
      throw Object.assign(new Error('WhatsApp send requires a target jid'), { code: 'bad-target' });
    }
    const text = typeof target.text === 'string' ? target.text : String(target.text ?? '');
    if (!text.trim()) {
      throw Object.assign(new Error('WhatsApp send requires non-empty text'), { code: 'bad-target' });
    }
    await client.sendText({ jid: target.jid }, text);
    return { sent: true, jid: target.jid };
  }
